...
export default async function DashboardPage() {
  ...
  try {
    data = await withTimeout(
      ...
      "El dashboard tardó demasiado en responder desde la base de datos."
    );
  } catch (error) {
    loadError =
  ...
  if (!data) {
    return (
      <div className="space-y-8">
        <section className="rounded-[32px] border border-amber-200 bg-[linear-gradient(135deg,#fff7ed_0%,#fffbeb_100%)] p-8 shadow-lg shadow-amber-100/70">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.25em] text-amber-700">Conexion degradada</p>
                <h1 className="mt-2 text-3xl font-semibold text-slate-900">El panel no pudo leer Azure SQL a tiempo.</h1>
              </div>
              <p className="max-w-3xl text-sm leading-6 text-slate-700">
                La app ya no se queda cargando indefinidamente: ahora mostramos este estado controlado cuando Prisma no logra
                consultar la base de datos o la conexion demora demasiado.
              </p>
              <div className="rounded-2xl border border-amber-200 bg-white/80 px-4 py-3 text-sm text-slate-700">
                <span className="font-semibold text-slate-900">Detalle:</span> {loadError}
              </div>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <article className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/60">
            ...
            <p className="mt-2 text-2xl font-semibold text-slate-900">Modo seguro</p>
            <p className="mt-2 text-sm text-slate-600">La interfaz responde, pero los datos operativos no se pudieron recuperar.</p>
          </article>
          <article className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/60">
            <p className="text-sm font-medium text-slate-500">Causa mas probable</p>
            <p className="mt-2 text-2xl font-semibold text-slate-900">Acceso BD</p>
            <p className="mt-2 text-sm text-slate-600">Firewall, credenciales o latencia alta hacia Azure SQL.</p>
          </article>
          <article className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg shadow-slate-200/60">
            <p className="text-sm font-medium text-slate-500">Comportamiento corregido</p>
            <p className="mt-2 text-2xl font-semibold text-slate-900">Sin espera infinita</p>
            <p className="mt-2 text-sm text-slate-600">El dashboard corta la espera y devuelve feedback útil al usuario.</p>
          </article>
        </section>
      </div>
    );
